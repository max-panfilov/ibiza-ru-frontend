...
Для статей используется класс `.article-content`, который определяет стили для всех HTML-элементов внутри. Это позволяет редакторам создавать богатый контент без необходимости задавать классы для каждого элемента.
